...
setGitAliase()
{
    ...
    git config --global alias.standup !"git log --reverse --branches --since=$(if [[ "Mon" == "$(date +%a)" ]]; then echo "last friday"; else echo "yesterday"; fi) --author=$(git config --get user.email) --format=format:'%C(cyan) %ad %C(yellow)%h %Creset %s %Cgreen%d' --date=relative"
    git config --global alias.rm-branches !"git branch -vv --merged | grep \": gone]\" | awk '{ print $1 }' | xargs -n 1 git branch -d"
    git config --global alias.aliases "config --get-regexp '^alias\\.'"
    git config --global alias.pr "!f() { exec az.cmd repos pr \"$@\"; }; f"
    git config --global alias.repo "!f() { exec az.cmd repos \"$@\"; }; f"

    git config --global alias.upgrade "!bash -c 'source <(curl -s https://raw.githubusercontent.com/eatatsys/git-init/master/setup.sh) -u'"
}
